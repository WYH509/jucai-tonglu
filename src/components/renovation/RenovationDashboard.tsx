  ...
  const fetchData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const isFallbackMode =
        billsRes.data.fallback ||
        materialsRes.data.fallback ||
        summaryRes.data.fallback;
          ...
          <div className="mb-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5">
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs text-amber-700">
                <p className="font-medium">CloudBase 数据库未配置</p>
                <p className="opacity-80 mt-0.5">请在 Render 后台添加 CLOUDBASE_ENV_ID 环境变量，当前仅显示演示数据</p>
              </div>
            </div>
          </div>
        )}
